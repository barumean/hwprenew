...
```bash
# 방법 1: 더블클릭 → 실행 창(GUI)
#   [파일 추가]/[폴더 추가]로 문서를 모아 [실행] → 로그 창에서 결과 확인
#   "서식 규칙 편집" 버튼으로 서식규칙.yaml을 바로 열 수 있음
python 표정리.py
...
- GUI를 띄울 수 없는 환경에서는 파일 선택 대화상자로 대신 동작합니다.
...
| F4 일괄(배치) 처리 | ✅ 구현 | GUI/CLI에서 여러 파일·폴더 순차 처리, 실패 격리 + 요약 |
| F5 원본 보호 | ✅ 구현 | `_정리본` 접미사로 사본 저장 |
| F6 변경 리포트 | 🔶 부분 | 콘솔/GUI 로그 출력만 (파일 저장 미지원) |
| F7 검사 전용 모드 | ❌ 미구현 | — |
| 간단 GUI (기획서 4단계) | ✅ 구현 | tkinter 실행 창: 파일/폴더 추가, 실행 로그, 규칙 파일 열기 |
