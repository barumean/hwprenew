...
| 파일 | 역할 |
|---|---|
| `표정리.py` | **단일 실행 파일**. 작업창(GUI) + 처리 엔진(CLI)이 한 파일에 통합. 규칙 파싱 → 표 분석 → 서식·너비 적용 → 후처리까지 전체 파이프라인 |
| `서식규칙.yaml` | 사용자가 편집하는 **서식 규칙 프리셋** (한국어 키). 작업창의 [서식 설정]으로도 편집 가능 |
| `requirements.txt` | 의존 패키지 (`pyyaml`, `pyhwpx`) |
| `서식/표준서식.sty` | 표준 서식(스타일) 원본. 한글에서 불러오면 아래 스타일 이름이 문서에 생깁니다 |
| `tests/test_표정리.py` | 자동 검증 95건. 한글 없이 실행: `python tests/test_표정리.py` |
| `서식 샘플.hwp` | 표준 서식의 참고용 샘플 문서 |
| `docs/기획서.md` | 프로젝트 기획서 (배경, 설계 결정, 로드맵) |
| `docs/implementation_plan.md` | 너비 조절 기능 구현 계획 (구현 완료) |

> [!NOTE]
> 이전에 별도 파일이던 `작업창.py`(GUI)는 `표정리.py`로 통합되어 삭제되었습니다. 작업창은 실제 처리를 `표정리.py` 자신을 하위 프로세스로 호출해 수행하므로 한글 COM 처리가 GUI와 분리되어 안정적입니다.
...
글자서식:        # 표 안 글자: 머리글행/본문셀의 스타일·글꼴·크기·진하게·정렬
표위치:          # 표가 놓인 문단의 정렬(= 표의 좌우 위치)·스타일
캡션:            # 표/그림틀의 번호 제목 — 사용, 위치(위·아래), 번호, 글자 서식
...
`서식/표준서식.sty`에 정의된 표 관련 스타일입니다. 규칙에서 **스타일 이름**으로 지정하면 글꼴·크기·정렬이 한 번에 맞춰집니다.

| 스타일 이름 | 크기 | 정렬 | 글꼴 | 쓰임 |
|---|---|---|---|---|
| `표위타이틀` | 11pt | 가운데 | KoPubWorld돋움체 Bold | 표 머리글 행 |
| `표왼타이틀` | 11pt | 배분 | KoPubWorld돋움체 Bold | 표 좌측 제목 열 |
| `표내용` | 11pt | 양쪽 | KoPubWorld돋움체 Medium | 표 본문 셀 |
| `표내용(가운데)` | 11pt | 가운데 | KoPubWorld돋움체 Medium | 가운데 정렬 본문 셀 |
| `표-번호제목` | 11pt | 양쪽 | KoPubWorld돋움체 Bold | 표 캡션 |
| `그림-번호제목` | 11pt | 양쪽 | KoPubWorld돋움체 Bold | 그림 캡션 |
| `표-그림위치` | 11pt | 양쪽 | KoPubWorld돋움체 Medium | 표·그림이 놓인 문단 |
| `자료` | 10pt | 양쪽 | KoPubWorld돋움체 Light | 출처 표기 |

> [!IMPORTANT]
> 스타일은 **그 이름이 대상 문서에 있을 때만** 적용됩니다. 없으면 조용히 건너뛰고 실행 로그에 어떤 스타일이 없는지 알려 줍니다. 한글에서 `서식/표준서식.sty`를 먼저 불러오면(서식 → 스타일 → 불러오기) 모든 문서에 동일하게 적용할 수 있습니다.
> 스타일 대신 `글꼴`·`크기`·`진하게`·`정렬`을 직접 지정할 수도 있습니다.
...
- `TableSpec`: 표마다 행/열 수, 셀 병합 정보(`(앵커행, 앵커열) → (행병합, 열병합)`), **캡션 유무**, 중첩 표 여부를 기록
- `read_style_ids()`: `header.xml`에서 **스타일 이름 → 번호** 표를 만든다. 규칙에 적은 스타일 이름을 번호로 바꿔 적용하기 위한 것으로, 문서에 없는 스타일은 실행 로그로 알리고 건너뛴다
...
- `apply_cell()`: COM의 `CellBorderFill`/`CellFill` 액션으로 테두리·배경을 적용하고, 규칙이 있으면 **셀 안 글자 서식**(스타일·글꼴·크기·진하게·정렬)까지 적용
- `extras()`: **표 위치**(표가 놓인 문단 정렬)와 **캡션**(없으면 생성 + 번호 + 위치 + 스타일)을 적용. 테두리 정리와 별개의 부가 작업이라 실패해도 표 전체를 실패로 만들지 않음
- `get_edit_width()` / `resize_table()`: 규칙이 `문서폭`/`고정값`이면 표·그림틀 너비를 편집 영역 폭(용지폭−좌우여백−제본여백) 또는 지정 mm로 조절.
  표 너비는 속성 대입이나 대화상자 액션으로는 바뀌지 않으므로(열 너비 합으로 재계산됨), 표를 HWPML로 추출해 셀 너비를 비율대로 고친 뒤 같은 자리에 다시 넣는다.
  **재삽입 과정에서 기존 표 컨트롤이 사라지므로 캡션·위치 적용은 반드시 너비 조절보다 먼저 수행**한다
...
UI와 처리 엔진이 **`표정리.py` 한 파일**에 통합되어 있습니다. 인자 없이 실행하면 작업창(GUI)이 열리고, 파일/폴더를 인자로 주면 명령줄로 바로 처리합니다.

```bash
# 방법 1 (권장): 작업창(GUI) — 더블클릭 또는
#   버튼으로 파일 선택 → [서식 설정]에서 규칙을 폼으로 편집 →
#   [정리 시작] → 로그·진행 표시 → [결과 폴더 열기]
python 표정리.py

# 방법 2: 명령줄 배치 — 파일/폴더를 여러 개 나열 (탐색기에서
#   파일들을 표정리.py 아이콘에 끌어다 놓는 것도 같은 방식)
python 표정리.py 문서1.hwp 문서2.hwpx 보고서폴더

# 방법 3: 진단 모드 — 한글 창을 띄운 채 실행
python 표정리.py --보기 문서.hwp
```

- 작업창의 [서식 설정]은 표준 서식이 기본으로 채워져 있고, 선 종류를 ‘없음’으로 고르면 그 선의 굵기·색 입력은 자동으로 비활성화됩니다. 색은 실제 색 미리보기와 [색 선택] 색상 선택기로 지정합니다.
- 폴더를 지정하면 바로 아래의 `.hwp`/`.hwpx`만 처리하며, 이 프로그램의 결과물(`*_정리본`)과 임시 파일은 자동으로 건너뜁니다.
- 한 파일이 실패해도 나머지는 계속 처리되고, 마지막에 성공/실패 요약이 출력됩니다.

**출력 결과**: 각 원본과 같은 폴더에 `<이름>_정리본.hwp(x)` 생성 (원본 무수정)
...
| F2 표 서식 일괄 적용 | ✅ 구현 | 외곽/안쪽선, 머리글행 배경·구분선, 그림틀, 머리글/본문 **글자 서식**(스타일·정렬) |
| F3 글자 서식 일괄 적용 | 🔶 표 범위 구현 | 표 안 셀·캡션에 스타일/글꼴/크기/진하게/정렬 적용. 본문 문단 전체 적용은 미구현 |
| 캡션(번호 제목) (기획서 외) | ✅ 추가 구현 | 표/그림틀에 캡션 자동 생성·번호·위치(위/아래)·스타일 |
| 표 위치 (기획서 외) | ✅ 추가 구현 | 표가 놓인 문단의 정렬로 표의 좌우 위치 지정 |
| F4 일괄(배치) 처리 | ✅ 구현 | CLI/드래그앤드롭에서 여러 파일·폴더 순차 처리, 실패 격리 + 요약 (작업창은 파일 1개씩) |
| F5 원본 보호 | ✅ 구현 | `_정리본` 접미사로 사본 저장 |
| F6 변경 리포트 | 🔶 부분 | 콘솔/GUI 로그 출력만 (파일 저장 미지원) |
| F7 검사 전용 모드 | ❌ 미구현 | — |
| 간단 GUI (기획서 4단계) | ✅ 구현 | `표정리.py` 내장 작업창: 파일 선택, 서식 설정 폼(색 미리보기·‘없음’ 비활성), 진행 로그, 결과 폴더 열기 |
