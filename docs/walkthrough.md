...
| 파일 | 역할 |
|---|---|
| `작업창.py` | **통합 GUI(작업창)**. 파일 선택·서식 설정 폼·진행 로그를 제공하고 `표정리.py`를 하위 프로세스로 호출 |
| `표정리.py` | **처리 엔진 겸 CLI**. 규칙 파싱 → 표 분석 → 서식·너비 적용 → 후처리까지 전체 파이프라인 |
...
UI는 **작업창.py 하나로 통합**되어 있습니다. `표정리.py`는 실제 처리를 담당하는 엔진 겸 명령줄 도구이고, 작업창.py가 그 위에 버튼 중심의 GUI를 얹어 `표정리.py`를 하위 프로세스로 호출합니다.

```bash
# 방법 1 (권장): 통합 작업창(GUI)
#   버튼으로 파일 선택 → [서식 설정]에서 자주 쓰는 규칙을 폼으로 편집 →
#   [정리 시작] → 로그·진행 표시 → [결과 폴더 열기]
python 작업창.py

# 표정리.py 를 더블클릭해도 됩니다 → 작업창.py 로 연결됩니다.
# (작업창.py 가 없으면 파일 선택 창으로 대신 동작)
python 표정리.py
...
| F7 검사 전용 모드 | ❌ 미구현 | — |
| 간단 GUI (기획서 4단계) | ✅ 구현 | 통합 작업창 `작업창.py`: 파일 선택, 서식 설정 폼, 진행 로그, 결과 폴더 열기 |
