...
> - **계획과 달라진 점**: 아래 3번의 `ctrl.Properties`에 Width를 대입하는 방식은 **표에는 적용되지 않는 것으로 확인**되었습니다(표 너비는 열 너비 합으로 레이아웃이 재계산됨). 실제 구현은 개체속성 대화상자와 같은 경로인 `TablePropertyDialog` 액션(너비 기준 = 절대값)을 사용하고, 적용 후 실제 너비를 검증합니다. 사진(그림 개체)은 계획대로 `Properties` 방식으로 동작합니다.
